...
@[simp]
theorem not_sym :  ¬ x = y →  ¬ y = x := by intro g; intro h; symm at h; contradiction
...
theorem list_nat_zero_to_aux_para1_elim :  List.map (fun x => x - i) (list_nat_zero_to_aux n i) = list_nat_zero_to n :=by
  have g:= @list_nat_zero_to_aux_para1_sub n 0 i; symm at g
  simp only [Nat.zero_add] at g; unfold list_nat_zero_to; exact g
